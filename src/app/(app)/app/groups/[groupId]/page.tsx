					...
					<div className="flex flex-col gap-1.5 min-w-0">
						...
						<div className="flex items-center gap-3 text-sm text-muted-foreground flex-wrap">
							...
							<span className="flex items-center gap-1.5">
								...
								{ended
									? `Ended ${format(new Date(group.endDate), "MMM d, yyyy")}`
									: `Ends ${format(new Date(group.endDate), "MMM d, yyyy")}`}
							</span>
							{!ended && (
								<Badge variant="outline" className="text-vibrant-green border-vibrant-green/30 text-xs">
									{formatDistanceToNow(new Date(group.endDate))} left
								</Badge>
							)}
							{ended && (
								...
								>
									...
									<div className="flex-1 min-w-0">
										<div className="flex items-center gap-2">
											<span className={cn("font-medium text-sm truncate", isCurrentUser && "text-vibrant-green")}>
												{member.user.name}
											</span>
											{isCurrentUser && (
