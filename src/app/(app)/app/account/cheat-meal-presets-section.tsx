...
export function CheatMealPresetsSection() {
    ...
    const runMigration = async () => {
      ...
      try {
        ...
        if (mounted && result.created > 0) {
          toast.success(
            `Criados ${result.created} itens salvos de refeições livres`,
          );
          await loadPresets();
        }
        if (mounted) {
  ...
  const handleLabelSave = async (preset: { id: string }, newLabel: string) => {
    try {
      await updateCheatMealPreset({ id: preset.id, label: newLabel, cascade: true });
      setPresets((prev) =>
          ...
          <AlertDialogContent className="dark:border-neutral-800 dark:bg-neutral-900">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="dark:text-neutral-400">
                Isso excluirá todos os seus itens salvos personalizados e
                restaurará as refeições livres padrão. Tem certeza?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
