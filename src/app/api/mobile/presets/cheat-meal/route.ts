...
export async function POST(request: Request) {
	...
	const maxOrder = await prisma.cheatMealPreset.aggregate({
		where: { userId: userId! },
		_max: { order: true },
	});
...
export async function PUT(request: Request) {
	...
	const preset = await prisma.cheatMealPreset.updateMany({
...
export async function DELETE(request: Request) {
	...
	// Null out presetId on related CheatMeals
	await prisma.cheatMeal.updateMany({
		where: { presetId: id, userId: userId! },
		data: { presetId: null },
	});

	await prisma.cheatMealPreset.deleteMany({ where: { id, userId: userId! } });
